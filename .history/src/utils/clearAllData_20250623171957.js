...
export const autoCleanForProduction = () => {
  // Verifica se está em produção REAL (deploy)
  const isProduction = process.env.NODE_ENV === 'production' && 
                      !window.location.hostname.includes('localhost') &&
                      !window.location.hostname.includes('127.0.0.1') &&
                      !window.location.hostname.includes('dev');
  
  // Flag para executar apenas uma vez
  const hasCleanedBefore = sessionStorage.getItem('data_cleaned_for_deploy');
  
  if (isProduction && !hasCleanedBefore) {
    console.log('🚀 Deploy limpo detectado - removendo dados salvos');
    const clearedCount = clearAllAppData();
    
    // Marcar como limpo nesta sessão
    sessionStorage.setItem('data_cleaned_for_deploy', 'true');
    
    if (clearedCount > 0) {
      console.log(`✅ Deploy limpo: ${clearedCount} itens removidos`);
      // Pequeno delay para o usuário ver o console
      setTimeout(() => {
        console.log('🔄 Recarregando para estado limpo...');
      }, 500);
    }
  } else if (!isProduction) {
    console.log('🔧 Ambiente de desenvolvimento - dados preservados');
  }
}; 