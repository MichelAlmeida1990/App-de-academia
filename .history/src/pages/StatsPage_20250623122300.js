...
const ProfilePage = () => {
  ...
  const handleAvatarUpload = async (event) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      alert('❌ Formato não suportado! Use apenas: JPEG, PNG ou WebP');
      return;
    }
    ...
    try {
      ...
      const newUserData = { ...userData, avatar: compressedImage };
      setUserData(newUserData);
      
      // Salvar automaticamente no localStorage
      await userDataService.saveUserProfile({ avatar: compressedImage });
  ...
  const compressImage = (file) => {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        // Definir tamanho máximo (400x400 para avatar)
        const maxSize = 400;
