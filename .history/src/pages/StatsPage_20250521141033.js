// src/pages/StatsPage.js - Versão corrigida para seu WorkoutContext
  ...
  const processFrequencyData = () => {
    ...
    filteredWorkouts.forEach(workout => {
      try {
        ...
        console.log(`Treino em ${format(date, 'EEEE', { locale: ptBR })} (${dayIndex}), contagem: ${frequencyData[dayIndex].workouts}`);
      } catch (error) {
      ...
      {activeTab === 'resumo' ? (
        <>
          ...
          <div className="p-2 mb-4 bg-gray-100 dark:bg-gray-800 rounded text-sm">
            <p>Total de treinos: {generalStats.totalWorkouts}</p>
            <p>Minutos totais: {generalStats.totalMinutes}</p>
            <p>Média por treino: {generalStats.averageMinutes} min</p>
          </div>
