...
const StatsPage = () => {
  const { workouts = [], getCompletedWorkouts, getWorkoutStatsByPeriod, loading } = useWorkout();
  ...
  const getMuscleGroupData = () => {
    ...
    completedWorkouts.forEach(workout => {
      // Extrair o grupo muscular do nome do treino ou usar a categoria
      const category = workout.category || 'Outros';
      muscleGroups[category] = (muscleGroups[category] || 0) + 1;
      
      // Se não houver categoria, tentar extrair do nome
