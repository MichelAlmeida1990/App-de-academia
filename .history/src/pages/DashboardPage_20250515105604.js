...
const DashboardPage = () => {
  ...
  useEffect(() => {
    try {
      ...
      const weekly = workouts && Array.isArray(workouts) ? getWeeklyWorkouts() : [];
      setWeeklyWorkouts(weekly);
      
      // Calcular estatísticas com verificações de nulidade
      const totalWorkouts = workouts && Array.isArray(workouts) ? workouts.length : 0;
      const totalDuration = workouts && Array.isArray(workouts)
        ? workouts.reduce((sum, workout) => sum + (workout.duration || 0), 0)
        : 0;
      const totalExercises = workouts && Array.isArray(workouts)
        ? workouts.reduce((sum, workout) => sum + (workout.exercises?.length || 0), 0)
        : 0;
      
      // Calcular streak (dias consecutivos)
      const streak = workouts && Array.isArray(workouts) ? calculateStreak(workouts) : 0;
