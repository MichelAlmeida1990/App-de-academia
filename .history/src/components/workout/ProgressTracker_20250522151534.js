...
const ProgressTracker = () => {
  ...
  // Função para obter a data atual correta
  const getToday = useCallback(() => {
    const now = new Date();
    console.log(`Data atual do sistema: ${format(now, 'dd/MM/yyyy HH:mm:ss')}`);
  ...
  // Obter a data atual a cada renderização
    ...
    try {
      ...
      if (!completedWorkouts || completedWorkouts.length === 0) {
        ...
        initialValues.weeklyActivity = last7Days.map((day) => {
          ...
          console.log(`Dia ${format(day, 'dd/MM/yyyy')} é hoje? ${isToday}`);
