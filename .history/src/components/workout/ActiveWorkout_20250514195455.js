...
const ActiveWorkout = () => {
  ...
  const handleCompleteSet = (setIndex, weight, reps) => {
    ...
    // Usar os valores dos inputs ou os valores fornecidos
    const actualWeight = weightInputs[setIndex] || weight;
    const actualReps = repsInputs[setIndex] || reps;
    
    completeSet(currentExerciseIndex, setIndex, actualWeight, actualReps);
    
    // Limpar os inputs após completar o set
    const newWeightInputs = {...weightInputs};
    const newRepsInputs = {...repsInputs};
    delete newWeightInputs[setIndex];
    delete newRepsInputs[setIndex];
  ...
  const handleWeightChange = (setIndex, value) => {
    setWeightInputs({...weightInputs, [setIndex]: value});
  };
  
  const handleRepsChange = (setIndex, value) => {
    setRepsInputs({...repsInputs, [setIndex]: value});
  };
                  ...
                  {set.completed ? (
                    ...
                    <div className="flex space-x-2">
                      ...
                      <input
                        ...
                        className="w-1/2 p-2 rounded border dark:bg-gray-600 dark:border-gray-500 dark:text-white"
                        min="0"
                        value={repsInputs[setIndex] || ''}
                        onChange={(e) => handleRepsChange(setIndex, e.target.value)}
                      />
                      <button
                        onClick={() => handleCompleteSet(setIndex, weightInputs[setIndex] || 70, repsInputs[setIndex] || 10)}
