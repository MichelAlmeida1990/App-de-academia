...
const ProgressTracker = () => {
  ...
  const { workouts, completedWorkouts } = useContext(WorkoutContext); // Usar diretamente o contexto para garantir acesso aos dados
  ...
  const getCompletedWorkouts = useCallback(() => {
    ...
    // Verificar o objeto completedWorkouts e extrair os IDs dos treinos concluídos
    const completedWorkoutIds = Object.keys(completedWorkouts || {}).filter(
      id => completedWorkouts[id] === true
    );
    
    console.log("IDs de treinos concluídos:", completedWorkoutIds);
    
    // Filtrar os treinos concluídos da lista de treinos
    const completed = workouts.filter(workout => 
      workout && workout.id && completedWorkoutIds.includes(String(workout.id))
    );
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log("Objeto completedWorkouts:", completedWorkouts);
  ...
  }, [workouts, completedWorkouts, getCompletedWorkouts, parseWorkoutDate, today]);
