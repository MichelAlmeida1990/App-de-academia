...
const ProgressTracker = () => {
  ...
  const generateDemoData = (realWorkouts) => {
    ...
    const weeklyActivity = last7Days.map(day => {
      // Usar dados reais se disponíveis, senão gerar aleatórios
      if (hasRealData) {
        const dayWorkouts = realWorkouts.filter(workout => {
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'overview' ? (
            ...
            >
              ...
              {progress.bodyPartDistribution.length > 0 && (
                <div className="bg-white dark:bg-gray-800/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                  ...
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        ...
                        >
                          {progress.bodyPartDistribution.map((entry, index) => (
              ...
              <div className="bg-white dark:bg-gray-800/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
  <h4 className="text-sm font-medium text-gray-600 dark:text-gray-300 mb-4">Taxa de Conclusão</h4>
    ...
    <div className="relative w-40 h-40">
      <svg className="w-full h-full" viewBox="0 0 100 100">
        ...
        <motion.circle 
          ...
          strokeWidth="10" 
          strokeDasharray={`${progress.completionRate * 2.51} 251`}
          strokeLinecap="round" 
          stroke="currentColor" 
          fill="transparent" 
          r="40" 
          ...
          animate={{ strokeDasharray: `${progress.completionRate * 2.51} 251` }}
          transition={{ duration: 1.5, ease: "easeOut" }}
          transform="rotate(-90 50 50)"
        />
      </svg>
      <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center">
        <span className="text-3xl font-bold text-gray-800 dark:text-white">{progress.completionRate}%</span>
      </div>
    </div>
  </div>
</div>
