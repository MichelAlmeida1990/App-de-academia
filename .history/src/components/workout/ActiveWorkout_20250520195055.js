...
const ActiveWorkout = () => {
  ...
  useEffect(() => {
    if (isResting && restTimeLeft > 0) {
      ...
      // Reproduzir som quando o descanso terminar
  ...
  const handleToggleExercise = async (exerciseIndex) => {
    ...
    if (!isCompleted) {
      // Efeito de confete pequeno quando concluir um exercício
      confetti({
        particleCount: 50,
        spread: 70,
        origin: { y: 0.6 }
  ...
  const handleFinishWorkout = async () => {
    ...
    // Grande efeito de confete para celebrar
    confetti({
      particleCount: 200,
      spread: 160,
      origin: { y: 0.6 }
  ...
  const startRest = (seconds) => {
    ...
    // Reproduzir som ao iniciar descanso
