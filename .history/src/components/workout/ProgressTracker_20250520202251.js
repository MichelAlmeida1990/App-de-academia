            ...
            >
              ...
              <AnimatePresence mode="wait">
                  ...
                  >
                    <div className="bg-white dark:bg-gray-800/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-3 text-center">
                        Frequência de treinos ao longo do período selecionado
                      </p>
                    </div>
                  </motion.div>
                )}
                  ...
                  >
                    <div className="bg-white dark:bg-gray-800/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                      <div className="flex justify-between items-center mb-4">
                        <h4 className="text-sm font-medium text-gray-600 dark:text-gray-300">Evolução de {METRICS.find(m => m.id === selectedMetric)?.label}</h4>
                        <div className="flex gap-2">
                          {METRICS.map(metric => (
                            <button
                              key={metric.id}
                              ...
                              className={`w-3 h-3 rounded-full border-2 ${
                                selectedMetric === metric.id
                                  ? `border-white bg-[${metric.color}]`
                                  : `border-[${metric.color}] bg-transparent`
                              }`}
                              style={{ 
                      ...
                      <div className="h-80">
                        {chartType === 'line' && (
                          <ResponsiveContainer width="100%" height="100%">
                            <LineChart
                              data={memoizedAdvancedData[selectedMetric]}
                              margin={{ top: 5, right: 30, left: 20, bottom: 5 }}
                            >
                              <CartesianGrid strokeDasharray="3 3" stroke={chartStyles.cartesianGrid.stroke} />
                              <XAxis dataKey="date" tick={{ fill: chartStyles.tick.fill }} />
                              ...
                              <Line 
                                ...
                                strokeWidth={2}
                                dot={{ r: 4, fill: METRICS.find(m => m.id === selectedMetric)?.color || "#3b82f6", strokeWidth: 0 }}
                                activeDot={{ r: 6, fill: METRICS.find(m => m.id === selectedMetric)?.color || "#3b82f6", stroke: chartStyles.tooltip.backgroundColor, strokeWidth: 2 }}
                                name={METRICS.find(m => m.id === selectedMetric)?.label || "Valor"}
                              />
                              <ReferenceLine y={0} stroke={chartStyles.cartesianGrid.stroke} />
                              <Brush dataKey="date" height={30} stroke={chartStyles.cartesianGrid.stroke} />
                            </LineChart>
                          </ResponsiveContainer>
                        )}
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-3 text-center">
                        Evolução de {METRICS.find(m => m.id === selectedMetric)?.label.toLowerCase()} ao longo do tempo
                      </p>
                    </div>
                  </motion.div>
                )}
                ...
                {advancedTab === 'comparison' && (
                  ...
                  >
                    <div className="bg-white dark:bg-gray-800/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                      <h4 className="text-sm font-medium text-gray-600 dark:text-gray-300 mb-4">Comparação de Desempenho</h4>
                      <div className="h-80">
                        <MemoizedRadarChart data={memoizedComparisonData} chartStyles={chartStyles} />
                      </div>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-3 text-center">
                ...
                {advancedTab === 'correlation' && (
                  ...
                  >
                    <div className="bg-white dark:bg-gray-800/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                      <h4 className="text-sm font-medium text-gray-600 dark:text-gray-300 mb-4">Correlação entre Métricas</h4>
                      <div className="h-80">
                        <MemoizedScatterChart data={memoizedScatterData} chartStyles={chartStyles} />
                      </div>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-3 text-center">
                        Correlação entre intensidade (X), volume (Y) e duração (tamanho)
                      </p>
                    </div>
                  </motion.div>
                )}
                ...
                {advancedTab === 'combined' && (
                  ...
                  >
                    <div className="bg-white dark:bg-gray-800/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                      <h4 className="text-sm font-medium text-gray-600 dark:text-gray-300 mb-4">Visão Combinada</h4>
                      <div className="h-80">
                        <MemoizedComposedChart data={memoizedComposedData} chartStyles={chartStyles} />
                      </div>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-3 text-center">
                        Combinação de frequência (barras), volume (área) e progresso (linha)
                      </p>
                    </div>
                  </motion.div>
                )}
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800/30">
                <h4 className="text-sm font-medium text-blue-700 dark:text-blue-400 mb-3">Insights</h4>
                <ul className="space-y-2 text-sm text-gray-700 dark:text-gray-300">
                  <li className="flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    {progress.weeklyTrend === 'up' ? (
                      <span>Sua frequência de treinos está aumentando! Continue assim para atingir seus objetivos mais rapidamente.</span>
                    ) : progress.weeklyTrend === 'down' ? (
                      <span>Sua frequência de treinos diminuiu recentemente. Tente estabelecer uma rotina mais consistente.</span>
                    ) : (
                      <span>Sua frequência de treinos está estável. Para ver resultados mais rápidos, considere aumentar a frequência.</span>
                    )}
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    {progress.streakDays > 3 ? (
                      <span>Excelente sequência de {progress.streakDays} dias! A consistência é chave para resultados duradouros.</span>
                    ) : progress.streakDays > 0 ? (
                      <span>Você está construindo uma sequência de {progress.streakDays} dias. Tente não quebrar o ritmo!</span>
                    ) : (
                      <span>Tente treinar hoje para começar uma nova sequência de dias consecutivos.</span>
                    )}
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    {progress.completionRate >= 80 ? (
                      <span>Taxa de conclusão excelente de {progress.completionRate}%! Você está muito comprometido com seus treinos.</span>
                    ) : progress.completionRate >= 60 ? (
                      <span>Taxa de conclusão de {progress.completionRate}% é boa, mas há espaço para melhorar sua consistência.</span>
                    ) : (
                      <span>Sua taxa de conclusão de {progress.completionRate}% pode melhorar. Tente priorizar seus treinos agendados.</span>
                    )}
                  </li>
                </ul>
              </div>
        ...
        <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <div className="flex gap-2">
            <button 
              ...
              className="px-3 py-1.5 text-xs font-medium rounded-lg bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-200"
            >
              Imprimir
            </button>
            <button 
              onClick={handleShareProgress}
              className="px-3 py-1.5 text-xs font-medium rounded-lg bg-blue-500 text-white"
            >
              Compartilhar
            </button>
          </div>
