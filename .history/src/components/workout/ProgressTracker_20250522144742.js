...
// Componente NoDataMessage inline
...
// Componente ProgressChart inline
const ProgressChart = ({ labels, datasets, height }) => {
  const theme = useTheme();
  
  if (!labels || !datasets || labels.length === 0) {
    return <NoDataMessage message="Sem dados suficientes para mostrar o gráfico" />;
  }
  
  // Preparar dados para o LineChart do MUI
...
const ProgressTracker = () => {
  const theme = useTheme();
  const { workouts, getCompletedWorkouts, completedWorkouts } = useWorkout();
  const [selectedTab, setSelectedTab] = useState(0);

  console.log("Processando dados de progresso...");

  // Função para obter treinos completados
  const getCompletedWorkoutsList = useCallback(() => {
  ...
  const processWorkoutData = useCallback(() => {
    const completedWorkouts = getCompletedWorkoutsList();
  ...
  }, [getCompletedWorkoutsList, theme.palette.primary.main, theme.palette.secondary.main]);
  ...
  const parseWorkoutDate = (workout) => {
    const dateString = workout.completedAt || workout.date;
  ...
  const hasNoData = useMemo(() => {
    ...
    if (!completedWorkouts || Object.keys(completedWorkouts).length === 0) return true;
