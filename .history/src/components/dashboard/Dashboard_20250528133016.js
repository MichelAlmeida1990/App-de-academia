...
import { useWorkout } from '../../context/WorkoutContext'; // Importando do contexto diretamente
import { FaDumbbell, FaChartLine, FaClipboardList, FaRunning } from 'react-icons/fa';
import { Link } from 'react-router-dom';

const Dashboard = () => {
  const { currentUser } = useAuth();
  const { workouts = [], loading: contextLoading, refreshWorkouts } = useWorkout(); // Usando o hook do contexto
  const [loading, setLoading] = useState(true);

  // Efeito para atualizar os treinos quando o componente for montado
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        // Atualiza os treinos do usuário atual
