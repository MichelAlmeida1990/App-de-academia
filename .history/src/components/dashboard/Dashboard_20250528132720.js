...
const Dashboard = () => {
  ...
  const { workouts = [], loading: contextLoading } = useContext(WorkoutContext);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Quando o contexto terminar de carregar, atualizamos nosso estado local
    if (!contextLoading) {
      setLoading(false);
    }
  }, [contextLoading, workouts]);
  ...
  const debugWorkout = (workout) => {
    console.log(`Treino: ${workout.name}, Data: ${workout.date}, Completado: ${workout.completed ? 'Sim' : 'Não'}`);
    if (workout.completedAt) {
      console.log(`Completado em: ${workout.completedAt}`);
    }
    return null;
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="mb-4 p-2 bg-gray-800 rounded-md" style={{display: 'none'}}>
          <p className="text-xs text-gray-400">Total de treinos: {workouts.length}</p>
          <div className="text-xs text-gray-400">
            {workouts.map(w => debugWorkout(w))}
          </div>
        </div>
      )}
