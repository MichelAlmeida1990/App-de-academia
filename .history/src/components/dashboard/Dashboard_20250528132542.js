...
const Dashboard = () => {
  ...
  const { workouts = [] } = useContext(WorkoutContext); // Obtém os treinos diretamente do contexto
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Simples efeito para garantir que a renderização ocorra após os dados serem carregados
    if (workouts) {
      setLoading(false);
    }
  }, [workouts]);
