...
// Importe o ExerciseCard (o caminho já está correto)
import ExerciseCard from './ExerciseCard';

const SearchExercises = () => {
  const context = useExercise();

  // Tratamento inicial para o contexto (o que você já tinha)
  if (!context) {
    return (
      <div className="flex justify-center items-center py-10">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500 mx-auto mb-4"></div>
          <p className="text-gray-600 dark:text-gray-300">Carregando contexto de exercícios...</p> {/* Mensagem mais específica */}
        </div>
      </div>
    );
  }
  ...
  const {
    ...
    savedExercises = [],
    toggleSavedExercise,
    isLoading: contextLoading // Usar um nome mais descritivo para evitar conflito com isLoading local
  } = context;

  const [searchTerm, setSearchTerm] = useState('');
  const [filteredExercises, setFilteredExercises] = useState([]);
  const [showFilters, setShowFilters] = useState(false);
  const [isLoading, setIsLoading] = useState(false); // Estado de carregamento para a busca/filtragem local
  ...
  // Inicializar com todos os exercícios quando carregarem do contexto
  // E re-filtrar quando os exercícios do contexto mudarem (ex: fetch inicial)
  useEffect(() => {
    if (exercises.length > 0) {
      handleSearch(); // Chame a função de busca para aplicar quaisquer filtros iniciais ou termo de busca
    }
  }, [exercises, searchTerm, bodyPartFilter, targetFilter, equipmentFilter]); // Adicionado dependências de filtro e termo de busca para re-filtrar automaticamente

  // Extrair opções de filtro únicas (executa apenas quando exercises mudam)
  useEffect(() => {
    if (exercises.length > 0) {
  ...
  const handleSearch = () => {
    if (exercises.length === 0) {
      setFilteredExercises([]); // Limpa se não há exercícios
      setIsLoading(false);
      return;
    }
    ...
    setTimeout(() => {
      ...
      // Atualizar filtros ativos após a busca
  ...
  const removeFilter = (filterType) => {
    ...
    // A chamada de handleSearch() no useEffect com as dependências já vai re-filtrar
  };

  // Limpar todos os filtros e termo de busca
  ...
  // Mostrar loading se o contexto ainda está carregando ou se a busca local está ativa
  if (contextLoading) {
    return (
      <div className="flex justify-center items-center py-10">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500 mx-auto mb-4"></div>
          <p className="text-gray-600 dark:text-gray-300">Carregando todos os exercícios do contexto...</p>
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 items-stretch"> {/* Adicionado items-stretch para cards de altura igual */}
            {filteredExercises.map((exercise) => (
              <ExerciseCard
                ...
                // Não é necessário passar onClick aqui se o card todo for um Link
              />
            ))}
          </div>
        ) : (
