...
export const WorkoutProvider = ({ children, navigate }) => {
  ...
  // Novo estado para armazenar recordes pessoais
  const [personalRecords, setPersonalRecords] = useState(() => {
    const savedRecords = localStorage.getItem('personalRecords');
    return savedRecords ? JSON.parse(savedRecords) : {};
  });
  
  // Novo estado para armazenar conquistas
  ...
  const checkForPersonalRecord = (exerciseName, weight) => {
  ...
  const checkWorkoutAchievements = () => {
