...
export const WorkoutProvider = ({ children }) => {
  ...
  // Função corrigida para criar treinos de exemplo específicos para o mês atual
  ...
  const getCompletedWorkouts = () => {
    ...
    // Se não houver treinos ou não for um array, retornar dados de exemplo
    if (!workouts || !Array.isArray(workouts) || workouts.length === 0) {
      console.log("Nenhum treino disponível, retornando dados de exemplo");
    ...
    if (completed.length === 0) {
      console.log("Nenhum treino concluído encontrado, retornando dados de exemplo");
      return createDemoWorkouts();
    }
  ...
  const getWorkoutStatsByPeriod = (period) => {
    ...
    if (filteredWorkouts.length === 0) {
      console.log("Nenhum treino encontrado no período, retornando dados de exemplo");
      return createDemoWorkouts();
    }
