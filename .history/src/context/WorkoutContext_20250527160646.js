...
// Não precisamos exportar useWorkout aqui se ele já está em src/hooks/useWorkout.js
...
export const WorkoutProvider = ({ children }) => {
  ...
  // Removendo completedWorkouts do estado aqui, pois o status de conclusão será no objeto workout
  ...
  useEffect(() => {
    const fetchWorkouts = async () => {
      ...
      try {
        setLoading(true);
        // Agora, getWorkouts deve retornar os treinos com o status 'completed' já definido
        const workoutData = LocalStorageService.getWorkouts(currentUser.uid);
        setWorkouts(workoutData);
        
        // Não precisamos mais carregar completedWorkouts separadamente se o status estiver no objeto workout
    ...
    fetchWorkouts();
  }, [currentUser]); // Dependência do currentUser para recarregar quando o usuário muda
  ...
  const deleteWorkout = async (workoutId) => {
    try {
      ...
      // Não precisamos mais manipular completedWorkouts separadamente aqui
  ...
  const createDemoWorkouts = () => {
    ...
    demoWorkouts.push({
      ...
      completed: true, // Marcar como concluído
      progress: 100,
    });
  ...
  // Função para obter todos os treinos concluídos, agora baseada na propriedade 'completed' do objeto workout
  const getCompletedWorkouts = () => {
    ...
    const completed = workouts.filter(workout => workout.completed === true); // Filtrar pela propriedade 'completed'
  ...
  const getInProgressWorkouts = () => {
    ...
    // Treinos em andamento são aqueles com progresso > 0 e < 100, e não marcados como concluídos
    return workouts.filter(workout => 
  ...
  const getWorkoutStatsByPeriod = (period) => {
    ...
    const filteredWorkouts = completedWorkoutsList.filter(workout => {
      if (!workout || !workout.completedAt) { // Agora sempre verificamos completedAt
        console.log(`Treino sem data de conclusão encontrado, ignorando:`, workout);
        return false;
      }
      
      const workoutDate = new Date(workout.completedAt); // Usar completedAt para treinos concluídos
  ...
  // Esta função agora também atualiza o objeto workout no array principal
  const toggleWorkoutCompletion = async (workoutId, completed) => {
    try {
      ...
      // Encontrar o treino no estado atual
      ...
      // Criar uma cópia do treino com as propriedades atualizadas
      const updatedWorkout = { ...workoutToUpdate };
      updatedWorkout.completed = completed;
      updatedWorkout.completedAt = completed ? new Date().toISOString() : null; // Define ou remove completedAt
      
      // Se estiver marcando como concluído, marque todos os exercícios como concluídos e defina progresso para 100
      if (completed) {
        updatedWorkout.exercises = updatedWorkout.exercises.map(ex => ({ ...ex, completed: true }));
        updatedWorkout.progress = 100;
      } else {
        // Se estiver desmarcando, desmarque todos os exercícios e zere o progresso
        updatedWorkout.exercises = updatedWorkout.exercises.map(ex => ({ ...ex, completed: false }));
        updatedWorkout.progress = 0;
      }

      // Chamar a função updateWorkout que já atualiza o localStorage e o estado 'workouts'
  ...
  const toggleExerciseCompletion = async (workoutId, exerciseIndex, completed) => {
    try {
      ...
      // Recalcular progresso geral do treino
      ...
      // Se todos os exercícios estiverem concluídos, marque o treino como concluído
      if (updatedWorkout.progress === 100) {
        ...
        updatedWorkout.completedAt = null; // Remova a data se não estiver 100%
      }
  ...
  return (
    <WorkoutContext.Provider 
      value={{ 
        ...
        getMuscleGroupStats,
        getGeneralStats,
        // createDemoWorkouts // Pode expor se quiser um botão para criar demos
      }}
    >
      {children}
    </WorkoutContext.Provider>
