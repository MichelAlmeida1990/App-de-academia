...
// Initialize Google Analytics (opcional) - DESABILITADO TEMPORARIAMENTE
let analytics;
try {
  // Só inicializar Analytics se tiver measurementId válido
  if (process.env.REACT_APP_FIREBASE_MEASUREMENT_ID && 
      process.env.REACT_APP_FIREBASE_MEASUREMENT_ID !== 'G-XXXXXXXXXX') {
    analytics = getAnalytics(app);
  } else {
    console.log('📊 Analytics desabilitado - measurementId não configurado');
  }
} catch (error) {
  if (process.env.NODE_ENV === 'development') {
