...
export const WorkoutProvider = ({ children }) => {
    ...
    if (isCompleted) {
      ...
      if (workout && workout.exercises) {
        ...
        workout.exercises.forEach((exercise, index) => {
          ...
          updatedCompletedExercises[exerciseKey] = true;
        });
  ...
  const toggleExerciseCompletion = (workoutId, exerciseIndex, isCompleted) => {
    ...
    // Criar uma nova cópia do objeto para garantir que a atualização seja detectada
    ...
    // Use a nova cópia para atualizar o progresso, não o estado anterior
    updateExerciseProgressInWorkout(workoutId, updatedCompletedExercises);
  };
  ...
  const updateExerciseProgressInWorkout = (workoutId, currentCompletedExercises = completedExercises) => {
    const workout = workouts.find(w => w.id === workoutId);
    ...
    workout.exercises.forEach((exercise, index) => {
      // Use o estado atualizado que foi passado como parâmetro
      const exerciseKey = `${workoutId}-${index}`;
      const isCompleted = currentCompletedExercises[exerciseKey] || false;
